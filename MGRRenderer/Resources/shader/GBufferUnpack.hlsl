...
TextureCube<float> _depthCubeMap : register(t1);
...
float unpackDepthOrthogonal(float2 uv)
{
	float depth = _gBuffer.Sample(_samplerState, uv).x;
	// 右手系で計算
	return (depth -_depthTextureProjection._m32) / _depthTextureProjection._m22;
}

float unpackDepthCubeMap(float3 uvw)
{
	float depth = _depthCubeMap.Sample(_samplerState, uvw);
	// 右手系で計算
	return -_depthTextureProjection._m32 / (depth + _depthTextureProjection._m22);
}
