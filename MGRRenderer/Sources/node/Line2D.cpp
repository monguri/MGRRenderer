...
void Line2D::renderForward(bool isTransparent)
{
	(void)isTransparent;
	...
	_renderForwardCommand.init([=]
	{
		...
		Logger::logAssert(SUCCEEDED(result), "Map failed, result=%d", result);
		Mat4 projectionMatrix = (Mat4::CHIRARITY_CONVERTER * Director::getCameraFor2D().getProjectionMatrix()).transpose();// 左手系変換行列はプロジェクション行列に最初からかけておく
